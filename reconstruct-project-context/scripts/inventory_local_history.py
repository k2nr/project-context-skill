        ...
        for item in paths:
            ...
            if (
                ...
                with path.open("ab") as mutation:
                    mutation.write(b"\n")
                os.environ.pop("PROJECT_CONTEXT_INVENTORY_TEST_MUTATE_SESSION", None)
            if fingerprint(path) != before:
                raise InventoryError("conversation source changed during collection")
        session_reports.append(
            {"provider": provider, "session_id": session_id, "records": index}
        )
    return (
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.include_conversations:
        current_sessions, _ = conversation_candidates(root, common)
        frozen_sessions = sorted(
            (item["provider"], item["session_id"], tuple(item["fingerprint"])) for item in sessions
        )
        current_fingerprints = sorted(
            (item["provider"], item["session_id"], tuple(item["fingerprint"]))
            for item in current_sessions
        )
        if current_fingerprints != frozen_sessions:
            raise InventoryError("conversation source set changed during collection")
