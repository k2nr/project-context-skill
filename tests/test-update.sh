...
mkdir -p "${release_root}/v0.4.0"
"${repository_root}/bin/package-skill" 0.4.0 "${release_root}/v0.4.0" >/dev/null

current_extract="${test_root}/current"
mkdir "$current_extract"
tar -xzf "${release_root}/v0.4.0/project-context-skill-v0.4.0.tar.gz" -C "$current_extract"

old_extract="${test_root}/old"
cp -R "$current_extract" "$old_extract"
sed 's/PROJECT_CONTEXT_VERSION="0.4.0"/PROJECT_CONTEXT_VERSION="0.3.0"/' \
...
old_archive="${release_root}/v0.3.0/project-context-skill-v0.3.0.tar.gz"
(
  cd "$old_extract"
...
run_update() {
  ...
  (
    ...
    PROJECT_CONTEXT_UPDATE_LATEST_VERSION=0.4.0 \
...
printf '%s\n' "$dry_run_output" | grep -Fq '"latest_version": "0.4.0"'
...
grep -Fq 'PROJECT_CONTEXT_VERSION="0.4.0"' \
  "${target}/.agents/skills/project-context/bin/project-context"
...
printf '%s\n' "$conflict_output" | grep -Fq 'local changes relative to v0.3.0'
