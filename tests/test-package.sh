...
if [ "$#" -eq 1 ]; then
  ...
  "$repository_root/bin/package-skill" 0.1.1 "$first_output" >/dev/null
  "$repository_root/bin/package-skill" 0.1.1 "$second_output" >/dev/null
  archive="${first_output}/project-context-skill-v0.1.1.tar.gz"
  cmp "$archive" "${second_output}/project-context-skill-v0.1.1.tar.gz"
  (
    cd "$first_output"
    if command -v sha256sum >/dev/null 2>&1; then
      sha256sum -c project-context-skill-v0.1.1.tar.gz.sha256
    else
      shasum -a 256 -c project-context-skill-v0.1.1.tar.gz.sha256
    fi
  )
fi
...
if [ -x "$debug_binary" ]; then
  ...
  release_root="${test_root}/release/v0.1.1"
  mkdir -p "$release_root"
  asset="${release_root}/project-context-v0.1.1-${target_arch}-${target_system}"
