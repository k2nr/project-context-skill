...
asset_name="project-context-v0.2.0-${target}"

create_release() {
  release_root=$1
  fixture_label=$2
  release_directory="${release_root}/v0.2.0"
...
fixture_a="${release_a}/v0.2.0/${asset_name}"
...
mv "${release_a}/v0.2.0" "${release_a}/v0.2.0.offline"
cached_output=$(
  PROJECT_CONTEXT_BOOTSTRAP_TESTING=1 \
  PROJECT_CONTEXT_RELEASE_BASE_URL="file://${release_a}" \
  PROJECT_CONTEXT_CACHE_DIR="${test_root}/cache" \
...
mv "${release_a}/v0.2.0.offline" "${release_a}/v0.2.0"

printf '%s\n' '#!/bin/sh' 'printf "corrupt-cache-ran\n"' > "${test_root}/cache/v0.2.0/${target}/project-context"
chmod 700 "${test_root}/cache/v0.2.0/${target}/project-context"
mv "${release_a}/v0.2.0" "${release_a}/v0.2.0.offline"
set +e
...
cached_checksum="${test_root}/cache/v0.2.0/${target}/project-context.sha256"
: > "$cached_checksum"
recovered_output=$(
  PROJECT_CONTEXT_BOOTSTRAP_TESTING=1 \
  PROJECT_CONTEXT_RELEASE_BASE_URL="file://${release_a}" \
