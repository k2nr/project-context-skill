...
The concrete GitHub repository is pending. Replace `OWNER/REPOSITORY` before
...
2. Download these assets for tag v0.1.0 into a private temporary directory:
   - https://github.com/OWNER/REPOSITORY/releases/download/v0.1.0/project-context-skill-v0.1.0.tar.gz
   - https://github.com/OWNER/REPOSITORY/releases/download/v0.1.0/project-context-skill-v0.1.0.tar.gz.sha256
...
4. If `gh` already exists, also run `gh attestation verify project-context-skill-v0.1.0.tar.gz --repo OWNER/REPOSITORY`. Stop if this verification fails. Do not install `gh` solely for this step; report that attestation verification was unavailable and continue with the checksum when `gh` is absent.
...
- Replace every `PENDING_GITHUB_REPOSITORY` and `OWNER/REPOSITORY` placeholder
  with the same official GitHub repository.
